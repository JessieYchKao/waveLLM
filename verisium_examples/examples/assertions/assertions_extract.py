...
# Query all signals in the entire design, returning any whose type is set
# to DeclarationType.ASSERTION.  Query up to 99 levels deep in the design
assertions = design_top.signals(ct.declaration_type == DeclarationType.ASSERTION, ct.depth < 99)

# Print index, name, full hierarchical path and number of transitions for each
# assertion to the screen
print('\n'.join(str(index) + ": \t" + str(a.name) + ", " + str(a.full_path) +
         ", #Transitions: " + str(a.transition_count) for index,a in enumerate(assertions)))
